...
export async function exportFullBackupFile(): Promise<boolean> {
  if (isTauriRuntime()) {
    ...
    const selected = await save({
      title: "导出完整加密账号配置",
      defaultPath: "codex-account-switcher-full.cswf",
      filters: [{ name: "Codex Account Switcher 完整备份", extensions: ["cswf"] }],
    });
  ...
  downloadBase64File(
    contentsBase64,
    "codex-account-switcher-full.cswf",
