...
use crate::auth::{
    ensure_chatgpt_tokens_fresh, load_accounts, refresh_chatgpt_tokens, update_account_plan_type,
};
use crate::types::{
...
async fn parse_usage_response(
    ...
    let usage = convert_payload_to_usage_info(&account.id, payload);
    sync_account_plan_type_from_usage(account, usage.plan_type.as_deref()).await;
    println!(
        "[Usage] {} - primary: {:?}%, plan: {:?}",
...
async fn sync_account_plan_type_from_usage(account: &StoredAccount, usage_plan_type: Option<&str>) {
    let Some(latest_plan_type) = usage_plan_type
        .map(str::trim)
    ...
    if account.plan_type.as_deref() == Some(latest_plan_type) {
        return;
    }

    println!(
        "[Usage] Detected plan_type change for {}: {:?} -> {}",
        account.name, account.plan_type, latest_plan_type
    );
    ...
    let refreshed_account = if is_active {
        match refresh_chatgpt_tokens(account).await {
            Ok(updated) => Some(updated),
            Err(err) => {
                println!(
                    "[Usage] Failed to refresh active account tokens for {} while syncing plan_type: {err}",
                    account.name
                );
                None
            }
    ...
    if refreshed_account
        .as_ref()
        .and_then(|updated| updated.plan_type.as_deref())
        == Some(latest_plan_type)
    {
        return;
    }

    if let Err(err) = update_account_plan_type(&account.id, latest_plan_type.to_string()) {
        println!(
            "[Usage] Failed to persist plan_type change for {}: {err}",
            account.name
        );
    }
}
