...
use crate::commands::{
    add_account_from_auth_json_text, add_account_from_file, add_current_auth_as_account,
    ...
    get_masked_account_ids, get_usage, get_workspace_auth_state,
...
pub fn run_lan_server(host: &str, port: u16) -> anyhow::Result<()> {
    ...
    println!("Codex Account Switcher web server listening on http://{address}");
...
async fn invoke_web_command(command: &str, payload: Value) -> Result<Value, String> {
    match command {
        ...
        "get_workspace_auth_state" => to_json(get_workspace_auth_state().await?),
        "add_account_from_file" => {
            let args: FileImportArgs = parse_args(payload)?;
            to_json(add_account_from_file(args.path, args.name).await?)
        }
        "add_current_auth_as_account" => {
            let args: NameArgs = parse_args(payload)?;
            to_json(add_current_auth_as_account(args.name).await?)
        }
