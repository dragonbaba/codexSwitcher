...
/// Synchronize only the stored plan type for an account.
pub fn update_account_plan_type(account_id: &str, plan_type: String) -> Result<bool> {
    let mut store = load_accounts()?;
    let changed = sync_account_plan_type_in_store(&mut store, account_id, &plan_type)?;
...
fn sync_account_plan_type_in_store(
...
#[cfg(test)]
mod tests {
    use super::sync_account_plan_type_in_store;
    use crate::types::{AccountsStore, StoredAccount};

    #[test]
    fn sync_account_plan_type_updates_store_when_changed() {
        let mut store = AccountsStore::default();
        let account = StoredAccount::new_api_key("demo".to_string(), "key".to_string());
        let account_id = account.id.clone();
        store.accounts.push(account);

        let changed =
            sync_account_plan_type_in_store(&mut store, &account_id, "plus").expect("sync");

        assert!(changed);
        assert_eq!(
            store.accounts[0].plan_type.as_deref(),
            Some("plus"),
            "plan_type should be updated in memory"
        );
    }

    #[test]
    fn sync_account_plan_type_is_noop_when_value_matches() {
        let mut store = AccountsStore::default();
        let mut account = StoredAccount::new_api_key("demo".to_string(), "key".to_string());
