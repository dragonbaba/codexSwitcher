...
use crate::auth::{
    add_account, create_auth_backup, create_chatgpt_account_from_refresh_token, get_active_account,
    get_codex_auth_file, import_from_auth_json, import_from_auth_json_contents,
    inspect_workspace_auth, load_accounts, remove_account, restore_auth_backup, save_accounts,
    switch_to_account, verify_current_auth_matches_account,
};
use crate::types::{
    AccountInfo, AccountsStore, AuthData, ImportAccountsSummary, StoredAccount,
    SwitchAccountResult, WorkspaceAuthState,
};
...
/// Add the current workspace auth.json as a managed account.
#[tauri::command]
pub async fn add_current_auth_as_account(name: String) -> Result<AccountInfo, String> {
    let auth_path = get_codex_auth_file().map_err(|e| e.to_string())?;
    let auth_path_str = auth_path
        .to_str()
        .ok_or_else(|| "Current auth.json path is not valid UTF-8".to_string())?;

    let account = import_from_auth_json(auth_path_str, name).map_err(|e| e.to_string())?;
    let stored = add_account(account).map_err(|e| e.to_string())?;
...
/// Read the current live auth.json state from the workspace.
#[tauri::command]
pub async fn get_workspace_auth_state() -> Result<WorkspaceAuthState, String> {
    let store = load_accounts().map_err(|e| e.to_string())?;
    Ok(inspect_workspace_auth(
        &store.accounts,
...
#[tauri::command]
pub async fn switch_account(account_id: String) -> Result<SwitchAccountResult, String> {
    let store = load_accounts().map_err(|e| e.to_string())?;
    let workspace_before =
        inspect_workspace_auth(&store.accounts, store.active_account_id.as_deref());
    ...
    let backup = create_auth_backup().map_err(|e| e.to_string())?;
    let backup_created = backup.backup_path.is_some();
    let backup_path = backup
        .backup_path
        .as_ref()
        .map(|path| path.display().to_string());

    if let Err(error) = switch_to_account(&account) {
        let rolled_back = restore_auth_backup(&backup).is_ok();
        let workspace_after =
            inspect_workspace_auth(&store.accounts, store.active_account_id.as_deref());
        return Ok(build_switch_result(
            false,
            &account,
            backup_created,
            backup_path,
    ...
    if let Err(error) = verify_current_auth_matches_account(&account) {
        let rolled_back = restore_auth_backup(&backup).is_ok();
        let workspace_after =
            inspect_workspace_auth(&store.accounts, store.active_account_id.as_deref());
        return Ok(build_switch_result(
            ...
            workspace_before,
            workspace_after,
            format!("Switch verification failed: {error}"),
        ));
    }

    let mut updated_store = store.clone();
    updated_store.active_account_id = Some(account.id.clone());
    if let Some(target) = updated_store
        .accounts
        .iter_mut()
    ...
    if let Err(error) = save_accounts(&updated_store) {
        ...
        return Ok(build_switch_result(
            ...
            workspace_before,
            workspace_after,
            format!("Failed to persist active account state: {error}"),
        ));
    }

    let killed_processes = restart_antigravity_processes();
    let workspace_after = inspect_workspace_auth(
        &updated_store.accounts,
        updated_store.active_account_id.as_deref(),
    );

    Ok(build_switch_result(
        true,
        &account,
        backup_created,
        backup_path,
...
fn build_switch_result(
    success: bool,
    account: &StoredAccount,
    backup_created: bool,
    backup_path: Option<String>,
    rolled_back: bool,
    killed_processes: usize,
    workspace_before: WorkspaceAuthState,
    workspace_after: WorkspaceAuthState,
    error: String,
) -> SwitchAccountResult {
    SwitchAccountResult {
        success,
        target_account_id: account.id.clone(),
        target_account_name: account.name.clone(),
        backup_created,
        backup_path,
        rolled_back,
        killed_processes,
...
fn restart_antigravity_processes() -> usize {
    let Ok(pids) = find_antigravity_processes() else {
        return 0;
    };
